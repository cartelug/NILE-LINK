...
(function(){
  ...
  NL.renderGrid=(el,list)=>{if(!el)return;el.innerHTML=list&&list.length?list.map((it,i)=>cardHTML(it,i)).join(''):'<div class="empty"><img src="'+href('assets/images/generated/empty-results.jpg')+'" alt="" class="empty-photo"><p style="font-weight:600">No listings match — try another category or search.</p></div>';if(window.NLi18n&&NLi18n.getLang()&&NLi18n.getLang()!=='en')document.dispatchEvent(new CustomEvent('nl:translate'));if(window.NLAnim)NLAnim.rescan()};
