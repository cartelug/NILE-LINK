...
(function(){
  ...
  let step=1;const state={cat:'',photos:[]};
  ...
  function bindPhotos(){
    $('#pPhotos').addEventListener('change',e=>{
      [...e.target.files].slice(0,6-state.photos.length).forEach(f=>{
        const r=new FileReader();r.onload=ev=>{state.photos.push(ev.target.result);renderPhotos()};r.readAsDataURL(f);
      });
      e.target.value='';
    });
  ...
  function renderPhotos(){
    $('#photoGrid').innerHTML=state.photos.map((src,i)=>'<div class="photo-thumb"><img src="'+src+'" alt=""><button data-rmphoto="'+i+'"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg></button></div>').join('');
  }
  ...
  function renderReview(){
    ...
    const it={id:'new',title:$('#pTitle').value||'Your listing',cat,key:state.cat,usd:+$('#pPrice').value||0,note,loc:$('#pLoc').value||'Your location',seller:'You',group:D.groupForCat(state.cat),badges:[],img:state.photos[0]||''};
    $('#reviewPreview').innerHTML=NL.cardHTML(it,0);
  }

  function publish(){
    const close=NL.toast('Publishing…',{type:'loading'});
    NL.api.listings.create({title:$('#pTitle').value.trim(),cat:D.CATLABEL[state.cat],key:state.cat,usd:+$('#pPrice').value,loc:$('#pLoc').value.trim(),desc:$('#pDesc').value.trim(),img:state.photos[0]||'',imgs:state.photos.slice(),note:state.cat==='property'?'/mo':''}).then(()=>{
      close();NL.toast('Listing published! 🎉',{type:'success'});
      setTimeout(()=>location.href='dashboard.html',900);
    });
  }
  function saveDraft(){
    const close=NL.toast('Saving draft…',{type:'loading'});
    NL.api.listings.saveDraft({title:$('#pTitle').value.trim(),cat:D.CATLABEL[state.cat],key:state.cat,usd:+$('#pPrice').value||0}).then(()=>{close();NL.toast('Draft saved',{type:'success'});setTimeout(()=>location.href='dashboard.html',800)});
  }
